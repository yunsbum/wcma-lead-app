...
(function () {
  ...
  function start(){ var done=0; function ready(){ if(++done>=4) boot(); }
    ...
    fetch('/api/promos',{credentials:'same-origin'}).then(function(r){return r.json();}).then(function(p){ if(Array.isArray(p)) PROMOS=p; }).catch(function(){}).then(ready);
    fetch('/api/schedule',{credentials:'same-origin'}).then(function(r){return r.json();}).then(function(s){ if(s&&typeof DB!=='undefined'){ if(s.schedule&&Object.keys(s.schedule).length) DB.schedule=s.schedule; if(Array.isArray(s.exceptions)) DB.exceptions=s.exceptions; } }).catch(function(){}).then(ready);
  }
