(function () {
  ...
  function conv(s,idnum){var created=s.createdAt?new Date(s.createdAt).toLocaleString():'';
    return {id:idnum,_sid:s._id,createdAt:s.createdAt||null,student:s.student||'(no name)',age:s.age,guardian:s.guardian||'',email:s.email||'',phone:s.phone||'',heard:s.source||'',program:s.program||'',when:s.when||'',src:s.source||'direct',paid:money(s),payStatus:s.payStatus||'none',status:s.status||'booked',notes:s.notes||'',created:created,archived:!!s.archived,deleted:!!s.deleted,promo:s.promo||'',activity:(s.activity&&s.activity.length)?s.activity:[{t:'Booking created',tm:created}]};}
