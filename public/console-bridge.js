(function () {
  ...
  function pushSync(){if(LOADING)return;try{
    ...
    payload.settings={stripeKey:st.stripeKey||'',sendgridKey:st.sendgridKey||'',fromEmail:st.fromEmail||'',notifyEmail:!!st.notifyEmail,notifySms:!!st.notifySms,schoolEmail:st.schoolEmail||'',schoolPhone:st.schoolPhone||'',twilioSid:st.twilioSid||'',twilioToken:st.twilioToken||'',twilioFrom:st.twilioFrom||''};
  ...
  function reflectStatus(){fetch('/api/settings',{credentials:'same-origin'}).then(function(r){return r.json();}).then(function(s){
    ...
    set('Stripe',s.stripeConnected);set('Email',s.emailConnected);set('Twilio',s.smsConnected);
  }).catch(function(){});}
  function install(){
    ...
    refresh();reflectStatus();if(typeof renderLinkQR==='function'){try{renderLinkQR();}catch(e){}}
